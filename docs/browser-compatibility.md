...
Once both scripts respond locally, switch to the MCP example and hit `GET /sse` to watch the endpoint announcement followed by keep-alive frames (every ~15s) generated by the Fetch adapter itself.【F:src/cloudflare/sse.ts†L22-L84】 Pair the stream with `POST /messages?sessionId=...` to send JSON-RPC payloads back into the server.【F:src/examples/cloudflareWorker.ts†L36-L69】
